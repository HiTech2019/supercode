...
func RandNewKey(len int) string {
	data := make([]byte, len)
	for i := 0; i < len; i++ {
...
func ReturnCode(secret string) uint32 {
	...
	//code := oneTimePassword([]byte(inputNoSpaces), toBytes(epochSeconds/30))
	code := oneTimePassword([]byte(key), toBytes(epochSeconds/10))
	// 此动态码剩余可用的秒数
...
func main() {
	key := "liuhangyu231231231" //RandNewKey(32)
	code := ReturnCode(key)
	fmt.Println(code)
}
